<script>
    ...
    const prod = "https://analysooorbackend-production.up.railway.app/"
    
    let initialWidth = 300
    $: dynamicStyle = `width: ${initialWidth}px;`;
    let isResizing = false;
    let showModal = "flex flex-col gap-2 w-[400px] h-fit bg-stone-100 mx-auto mt-48 p-6 rounded-lg shadow-2xl";
    let showLoading = "hidden"
    let showCanvas = "hidden"

    let memeDashboard = "w-full text-sm text-stone-600 transition delay-100 px-4 py-[7px] rounded-lg hover:bg-stone-200 hover:cursor-pointer"
    let trendDashboard = "w-full text-sm text-stone-600 transition delay-100 0 px-4 py-[7px] rounded-lg hover:bg-stone-200 hover:cursor-pointer"
    ...
    function openFileInput() {
        inputFile = "z-50 absolute visible bg-red-400"
    }

    async function keyDown(event) {
        if (event.key == "Enter") {
            console.log("Enter key pressed")
            const usernameArrays = performUsernameLogic()
            showModal = "hidden"
            showLoading = "my-auto mx-auto loading"
            console.log(encodeURIComponent(usernameArrays))
            const response = await fetch(`${prod}twitter?username=${JSON.stringify(encodeURIComponent(usernameArrays))}`, {
                method: "POST",
                mode: "no-cors",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify(usernameArrays)
            })
    ...
    async function test(event) {
        if (event.key == "Enter") {
            ...
            if (body.includes("Error")) {
                ...
                chart.update()
                showCanvas = "items-center flex flex-row ml-[300px] visible my-auto my-auto w-[60%] h-[450px]"
                showLoading = "hidden"
                successMessage = "mx-auto text-green-400 mt-10 shadow-xl"
            }
        }
    ...
    function performDashboardSearch() {
        ...
        if (!searchValue) {
            memeDashboard = "w-full text-sm text-stone-600 transition delay-100 px-4 py-[7px] rounded-lg hover:bg-stone-200 hover:cursor-pointer" //visible
            trendDashboard = "w-full text-sm text-stone-600 transition delay-100 0 px-4 py-[7px] rounded-lg hover:bg-stone-200 hover:cursor-pointer"
            return
        }
...
<main class="flex flex-row bg-white w-full h-screen overflow-scroll">
    ...
    <div class="fixed resizable z-50 bg-white transition delay-100 flex flex-col md:start-0 h-screen py-4 px-2 border-r-[1px] border-stone-300 shadow-2xl"  style={dynamicStyle} bind:this="{resizableDiv}">
        ...
        <div class={memeDashboard}>Meme predictions</div>
        <div class={trendDashboard}>Trend predictions</div>
    ...
    <div class={showModal}>
        ...
        <button class="w-full p-1 py-[5.5px] rounded-lg mx-auto bg-stone-300 font-semibold transition delay-150 hover:bg-blue-100">Add a CSV file</button>
        <input class={inputFile} type="file" accept=".csv" on:click={openFileInput} />
        <button class="w-full mt-1 p-1 py-[5.5px] rounded-lg mx-auto bg-stone-200 font-semibold text-blue-500 transition delay-150 hover:bg-blue-100" on:click={test}>Submit</button>
